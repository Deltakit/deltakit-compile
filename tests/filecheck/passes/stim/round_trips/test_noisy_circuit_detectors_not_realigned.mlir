...
// Tags, noise, gates, measures and simple nested repeats. Tags not extracted so should be kept. Detectors not realigned so ordering should be preserved.
...
// CHECK-NEXT:        %0 = stim.qubit_alloc 0 {stim.tag = "{\22array\22: [\22#Z\22, 1, \22#1\22, {\22key\22: null}\\C}"} -> !stim.qubit
...
// CHECK-NEXT:        stim.reset Z (%0, %3, %1) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
...
// CHECK-NEXT:        stim.depolarize1 <0.001> (%2, %4) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
// CHECK-NEXT:        stim.depolarize2 <0.002{{(00000[0-9]*)?}}> (%2, %4) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
// CHECK-NEXT:        %5, %6 = stim.measure X (%2, %4) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"} -> i1, i1
// CHECK-NEXT:        stim.tick

  stim.detector <[1.0, 0.0, 0.0]> (%5 : i1) {stim.tag = "{\22my_type\22: \22#!test.type<\5C\22test\5C\22>\22}"}
  stim.detector <[3.0, 0.0, 2.0]> (%6 : i1)

// CHECK-NEXT:        stim.detector <[1.0, 0.0, 0.0]> (%5 : i1) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
...
// CHECK-NEXT:        %7, %8 = stim.repeat {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"} 2 (%5, %6 : i1, i1) -> i1, i1 {
...
// CHECK-NEXT:          stim.pauli_channel_1 <0.01, 0.02, 0.03> (%2, %4) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
...
// CHECK-NEXT:        stim.tick
  ...
  stim.correlated_error <0.1> [Z, X, Z, Z] (%0, %3, %1, %2) {stim.tag = "{\22my_type\22: \22#!test.type<\5C\22test\5C\22>\22}"}
...
// CHECK-NEXT:            stim.tick
// CHECK-NEXT:            stim.correlated_error <0.19999999999999998> [X] (%1) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
// CHECK-NEXT:            stim.else_correlated_error <0.125> {{\[Z, X, Z, Z\] \(%0, %3, %1, %2\)|\[Z, Z, X, Z\] \(%1, %0, %3, %2\)}} {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
// CHECK-NEXT:            stim.pauli_channel_2 <0.001, 0.002, 0.003, 0.004, 0.005, 0.006, 0.007, 0.008, 0.009, 0.01, 0.011, 0.012, 0.013, 0.014, 0.015> (%2, %4) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
// CHECK-NEXT:            stim.detector <[1.0, 0.0, 0.0]> (%7, %13 : i1, i1)
// CHECK-NEXT:            stim.detector <[3.0, 0.0, 0.0]> (%8, %14 : i1, i1)
// CHECK-NEXT:            stim.observable_include <0> (%17 : i1) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}

// RUN: deltakit_compile compile-passes -t %s -p stim-import-pipeline --pass-args '{"verify_between_passes": true, "extract_tags_to_attributes": false, "realign_detectors": false}' -O %t && filecheck %s --input-file %t

// Tags, noise, gates, measures and simple nested repeats. Detectors not moved.
...
// CHECK-NEXT:    %0 = qcore.alloc_qubit<coords = [(4.0, 0.0)], ids = [0]> {stim.tag = "{\22array\22: [\22#Z\22, 1, \22#1\22, {\22key\22: null}\\C}"} -> !qcore.qubit
...
// CHECK-NEXT:      qref.reset<Z> (%11, %14, %12) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
...
// CHECK-NEXT:      qref.pauli_noise<X = 0.0003333333333333333, Y = 0.0003333333333333333, Z = 0.0003333333333333333> (%13, %15) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
// CHECK-NEXT:      qref.pauli_noise<IX = 0.00013333333333333334, IY = 0.00013333333333333334, IZ = 0.00013333333333333334, XI = 0.00013333333333333334, XX = 0.00013333333333333334, XY = 0.00013333333333333334, XZ = 0.00013333333333333334, YI = 0.00013333333333333334, YX = 0.00013333333333333334, YY = 0.00013333333333333334, YZ = 0.00013333333333333334, ZI = 0.00013333333333333334, ZX = 0.00013333333333333334, ZY = 0.00013333333333333334, ZZ = 0.00013333333333333334> (%13, %15) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
// CHECK-NEXT:      %17, %18 = qref.measure<X> (%13, %15) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"} -> i1, i1
// CHECK-NEXT:      qec.measurement_round(%17, %18 : i1, i1)
// CHECK-NEXT:      %19 = qec.detector<[1.0, 0.0]> (%17) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
...
// CHECK-NEXT:      %27, %28, %29, %30, %31, %32, %33, %34 = qstruct.repeat<2> (%19, %24, %25, %22, %23, %26, %17, %18 : !qec.detector_ref, !qec.detector_ref, !qec.detector_ref, !qec.detector_ref, !qec.detector_ref, !qec.detector_ref, i1, i1) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"} -> !qec.detector_ref, !qec.detector_ref, !qec.detector_ref, !qec.detector_ref, !qec.detector_ref, !qec.detector_ref, i1, i1 {
...
// CHECK-NEXT:        qref.gate<#qcore.gate.cz> (%13, %12, %15, %14)
// CHECK-NEXT:        qref.gate<#qcore.gate.cz> (%13, %14, %15, %11)
// CHECK-NEXT:        qref.pauli_noise<X = 0.01, Y = 0.02, Z = 0.03> (%13, %15) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
...
// CHECK-NEXT:      %52, %53, %54 = qref.measure<Z> (%11, %14, %12) -> i1, i1, i1
// CHECK-NEXT:      qec.measurement_round(%52, %53, %54 : i1, i1, i1)
// CHECK-NEXT:      qstruct.parallel<TOP> -> {
// CHECK-NEXT:        qref.pauli_noise<IX = 0.19999999999999998, ZI = 0.1> (%11, %12) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
// CHECK-NEXT:        qstruct.yield
// CHECK-NEXT:      } {
// CHECK-NEXT:        qref.pauli_noise<IX = 0.001, IY = 0.002, IZ = 0.003, XI = 0.004, XX = 0.005, XY = 0.006, XZ = 0.007, YI = 0.008, YX = 0.009, YY = 0.01, YZ = 0.011, ZI = 0.012, ZX = 0.013, ZY = 0.014, ZZ = 0.015> (%13, %15) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"}
// CHECK-NEXT:        qstruct.yield
// CHECK-NEXT:      }
// CHECK-NEXT:      %55 = qec.detector<[1.0, 0.0]> (%33, %50)
// CHECK-NEXT:      %56 = qec.detector<[3.0, 0.0]> (%34, %51)
// CHECK-NEXT:      qec.detector_round(%20, %21, %28, %29, %55, %56)
// CHECK-NEXT:      %57 = qec.observable_include(%16) using (%54) {stim.tag = "{\22my_type\22: \22#!test.type<\\\22test\\\22>\22}"} -> !qec.observable
// CHECK-NEXT:      %58 = qec.detector<[1.0, 0.0]> (%50, %54, %53)
// CHECK-NEXT:      %59 = qec.detector<[3.0, 0.0]> (%51, %52, %53)
// CHECK-NEXT:      qec.detector_round(%30, %31, %58, %59)
// CHECK-NEXT:      qec.detector_round(%32)
// CHECK-NEXT:      qstruct.yield %57, %11, %12, %13, %14, %15 : !qec.observable, !qcore.qubit, !qcore.qubit, !qcore.qubit, !qcore.qubit, !qcore.qubit
// CHECK-NEXT:    }

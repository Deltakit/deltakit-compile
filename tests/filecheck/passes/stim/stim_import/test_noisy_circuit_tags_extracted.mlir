// RUN: deltakit_compile compile-passes -t %s -p stim-import-pipeline --pass-args '{"verify_between_passes": true, "extract_tags_to_attributes": true}' -O %t && filecheck %s --input-file %t
...
// CHECK-NEXT:      }
  ...
  stim.detector <[1.0, 0.0, 0.0]> (%7, %17 : i1, i1) {stim.tag = "434"}
  ...
  stim.detector <[1.0, 0.0, 0.0]> (%17, %21, %20 : i1, i1, i1) {stim.tag = "3424"}
...
// CHECK-NEXT:      %52 = qec.detector<[1.0, 0.0]> (%33, %50) {stim.tag = "434"}
...
// CHECK-NEXT:      %57 = qec.detector<[1.0, 0.0]> (%50, %56, %55) {stim.tag = "3424"}
